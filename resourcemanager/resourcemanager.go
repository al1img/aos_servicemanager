...
func (resourcemanager *ResourceManager) AllocateDevice(instanceID string, name string) error {
	resourcemanager.Lock()
	defer resourcemanager.Unlock()

	log.WithFields(log.Fields{"instanceID": instanceID, "device": name}).Debug("Allocate device")

	if resourcemanager.boardConfigError != nil {
		return aoserrors.Wrap(resourcemanager.boardConfigError)
	}

	deviceResource, err := resourcemanager.getAvailableDevice(name)
	if err != nil {
		return aoserrors.Wrap(err)
	}

	// get list of instances that are using this device
	instances := resourcemanager.allocatedDevices[name]

	if contains(instances, instanceID) {
		log.WithFields(log.Fields{
			"device": name, "instanceID": instanceID,
	...
	// len(instances) < deviceResource.SharedCount: provide device until list less then sharedCount value
	if deviceResource.SharedCount != 0 && len(instances) >= deviceResource.SharedCount {
		return aoserrors.New("device shared count limit reached")
	}

	resourcemanager.allocatedDevices[name] = append(instances, instanceID)
