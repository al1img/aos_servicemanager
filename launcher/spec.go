...
func (spec *runtimeSpec) save(fileName string) error {
	log.WithField("fileName", fileName).Debug("Save runtime spec")
	...
	if err == nil {
		...
		_ = filepath.Walk(hostPath, func(path string, fileInfo os.FileInfo, _ error) error {
			...
			if specDevice, err = devices.DeviceFromPath(path, permissions); err != nil {
				// skip device
				log.WithField("device", path).Warnf("Skip device: %s", err)
...
func (launcher *Launcher) createRuntimeSpec(instance *instanceInfo, service *serviceInfo) (*runtimeSpec, error) {
	spec := &runtimeSpec{
	...
	instance.envVars = launcher.getInstanceEnvVars(instance.InstanceInfo)
	spec.mergeEnv(instance.envVars)
	...
	if err := spec.applyImageConfig(service.imageConfig); err != nil {
		return nil, aoserrors.Wrap(err)
	}

	if err := spec.applyServiceConfig(service.serviceConfig); err != nil {
		return nil, aoserrors.Wrap(err)
	}

	if err := spec.save(filepath.Join(instance.runtimeDir, runtimeConfigFile)); err != nil {
		return nil, aoserrors.Wrap(err)
	}
